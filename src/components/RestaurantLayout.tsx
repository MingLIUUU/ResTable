...
const RestaurantLayout: React.FC = () => {
 ...
 // 添加新的绘制函数来展示不同类型的桌子
 const drawTableOption = (ctx: CanvasRenderingContext2D, type: TableType, x: number, y: number) => {
  ctx.save();
  ctx.translate(x, y);
  ...
  const renderTableOptions = () => {
    ...
    const ctx = canvas.getContext('2d');
    if (ctx) {
      drawTableOption(ctx, 'square', 25, 25);
      drawTableOption(ctx, 'diamond', 75, 25);
      drawTableOption(ctx, 'round', 125, 25);
    }
    return canvas;
  };
        ...
        if (!chairUpdated && !tableDeleted && !isNearby) {
          ...
          const addTableToRoom = (room: Room): Room => {
            ...
            if (isPointInPolygon(gridAlignedX, gridAlignedY, room.walls.map(([x1, y1]) => ({ x: x1, y: y1 })))) {
              ...
              return {
                ...room,
                tables: [...room.tables, { x: gridAlignedX, y: gridAlignedY, chairs: [true, true, true, true] }]
              };
            }
        ...
        {currentTool === 'table' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <img src={renderTableOptions().toDataURL()} alt="Table options" />
              <select value={selectedTableType} onChange={(e) => setSelectedTableType(e.target.value as TableType)}>
                <option value="square">square table</option>
                <option value="diamond">diamon table</option>
                <option value="round">round table</option>
              </select>
            </div>
            <div>
              <button onClick={() => {/* 添加桌子逻辑 */}}>Add</button>
              <button onClick={() => {/* rotate桌子逻辑 */}}>Rotate</button>
